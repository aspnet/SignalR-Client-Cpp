...
TEST(connection_impl_start, start_fails_if_negotiate_response_does_not_have_request_transfer_format)
{
    std::shared_ptr<log_writer> writer(std::make_shared<memory_log_writer>());
    ...
    auto mre = manual_reset_event<void>();
    connection->start(transfer_format::binary, [&mre](std::exception_ptr exception)
        {
            mre.set(exception);
        });
    ...
    {
        ASSERT_STREQ("The server does not support WebSockets with the requested transfer format 'binary'.", e.what());
    }
}

TEST(connection_impl_start, start_works_if_negotiate_response_has_requested_transfer_format)
{
    std::shared_ptr<log_writer> writer(std::make_shared<memory_log_writer>());
...
TEST(connection_impl_stop, triggers_http_send_token)
{
    ...
    connection->start(transfer_format::text, [&start_mre](std::exception_ptr exception)
        {
