...
namespace signalr
{
    ...
    {
        hub_message_base(message_type message_type) : message_type(message_type) {}
    ...
    {
        hub_invocation_message(const std::string& invocation_id, enum message_type message_type)
    ...
    struct invocation_message : hub_invocation_message
    {
        invocation_message(const std::string& invocation_id, const std::string& target,
            ...
            : hub_invocation_message(invocation_id, message_type::invocation), target(target), arguments(args), stream_ids(stream_ids)
        { }
    ...
    struct completion_message : hub_invocation_message
    {
        completion_message(const std::string& invocation_id, const std::string& error, const signalr::value& result)
            : hub_invocation_message(invocation_id, message_type::completion), error(error), result(result)
        { }
    ...
    {
        ping_message() : hub_message_base(message_type::ping) {}
    };
    ...
    public:
        ...
        virtual transfer_format transfer_format() const = 0;
